				 ...
				 const std::string & title) 
{
	...
	{
	  tmp << iColumn << " " << iRow << " " << static_cast <
	    float >(ucPicBuf[iIndex++]) <<
	    std::endl;
  ...
  bool Gnuplot::file_exists (const std::string & filename, int mode) 
{
	...
	runtime_error
	("In function \"Gnuplot::file_exists\": mode has to be an integer between 0 and 7");
...
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__TOS_WIN__)
    char
    name[] = "gnuplotiXXXXXX";	//tmp file in working directory
#elif defined(unix) || defined(__unix) || defined(__unix__) || defined(__APPLE__)
    char
    name[] = "/tmp/gnuplotiXXXXXX";	// tmp file in /tmp
#endif	/*  */
