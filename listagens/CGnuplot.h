...
private:
  //----------------------------------------------------------------------------------- Atributos
  FILE *
    gnucmd;			///< Ponteiro para stream que escreve no pipe.
  bool
    valid;			///< Flag que indica se a sessao do gnuplot esta valida.
  bool
    two_dim;			///< true = verdadeiro = 2d, false = falso = 3d.
  int
    nplots;			///< Numero de graficos (plots) na sessao.
  std::string
    pstyle;			///< Estilo utilizado para visualizacao dads funcoes e dados.
  std::string
    smooth;			///< interpolate and approximate data in defined styles (e.g. spline).
  std::vector <
    std::string >
    tmpfile_list;		///< Lista com nome dos arquivos temporarios.

  //----------------------------------------------------------------------------------------flags
  bool
    fgrid;			///< 0 sem grid,        1 com grid
  bool
    fhidden3d;			///< 0 nao oculta,      1 oculta
  bool
    fcontour;			///< 0 sem contorno,    1 com contorno
  bool
    fsurface;			///< 0 sem superficie,  1 com superficie
  bool
    flegend;			///< 0 sem legendad,    1 com legenda
  bool
    ftitle;			///< 0 sem titulo,      1 com titulo
  bool
    fxlogscale;			///< 0 desativa escala log, 1 ativa escala log
  bool
    fylogscale;			///< 0 desativa escala log, 1 ativa escala log
  bool
    fzlogscale;			///< 0 desativa escala log, 1 ativa escala log
  bool
    fsmooth;			///< 0 desativa,        1 ativa 

  //---------------------------------------------------------------------------------------------
  // Atributos estaticos (compartilhados por todos os objetos)
  static int
    tmpfile_num;		///< Numero total de arquivos temporarios (numero restrito).
  static
    std::string
    m_sGNUPlotFileName;		///< Nome do arquivo executavel do gnuplot. 
  static
    std::string
    m_sGNUPlotPath;		///< Caminho para executavel do gnuplot.
  static
    std::string
    terminal_std;		///< Terminal padrao (standart), usado para visualizacoes.
  ...
  /// @brief Cria arquivo temporario.
  std::string CreateTmpFile (std::ofstream & tmp)
  {
    return create_tmpfile (tmp);
  }
  ...
  /// @brief Checa se o arquivo existe.
  static bool
  FileExists (const std::string & filename, int mode = 0)
  {
...
public:
  ...
  static void
  Terminal (const std::string & type)
  {
  ...
  /// @brief Envia comando para o gnuplot.
  Gnuplot & Command (const std::string & cmdstr)
  {
    return cmd (cmdstr);
  }
  ...
  {
    return set_isosamples (isolines);
  }
  ...
  /// @brief Ativa escala logaritma do eixo y (logscale nao e setado por default).
  Gnuplot & ZLogscale (const double base = 10)
  {
  ...
  Gnuplot & PlotFile (const std::string & filename,
		      ...
		      const int column_z = 3, const std::string & title = "")
  {
    return plotfile_xyz (filename, column_x, column_y, column_z);
  }
...
Problema:
    ...
    g0.Title().Replot();
      wait_for_key(); //sleep(SLEEP_LGTH);


      g0.Title("titulo ooo").Replot();
      wait_for_key(); //sleep(SLEEP_LGTH);

      wait_for_key(); //sleep(SLEEP_LGTH);

      g0.unset_title().Replot();
...
3 condicoes:
1- facil - ativa/desativa (requer um flag)
    Hidden();      // ativa    ocultar
    Hidden(int 1); // ativa    ocultar
    Hidden(int 0); // desativa ocultar
2- media - recebe uma string com valor pre-definido.
   requer um flag e uma string
    Smooth(int 0);  // desativa (requer um flag)
    Smooth(int 1);  // ativa
    Smooth(" asd"); // ativa e seta o conteudo da variavel, formato do Smooth
3- 
